...
import spacegame.gui.screen.GuiFinishedGame;
...
public class EntitySpawner extends EntityEnemy {
	...
	public void onDespawn() {
		...
		if(base instanceof EntityPlanet) {
			...
			IngameState.getInstance().entityManager.player.currentHealth = IngameState.getInstance().entityManager.player.maxHealth;
			for(EntityBase entity : IngameState.getInstance().entityManager.ingameEntities) {
				if(entity instanceof EntityPlanet && entity.id != base.id) {
					EntityPlanet planet = (EntityPlanet) entity;
					if(!World.getPlanetInfo(planet).liberated) {
						return;
					}
				}
			}
			IngameState.getInstance().guiHierarchy.openGui(new GuiFinishedGame());
		}
	}
